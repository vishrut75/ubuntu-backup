...
def curOdom(x,y,theta):
    ...
    r_y = l*sin(ang)
    cur = Odometry()
    cur.pose.pose.position.x = r_x
    cur.pose.pose.position.y = r_y
    cur.pose.pose.position.z = 0
    cur.pose.pose.orientation.x,cur.pose.pose.orientation.y,cur.pose.pose.orientation.z,cur.pose.pose.orientation.w = quaternion_from_euler(0,0,r_th)
    return cur
...
while not rospy.is_shutdown():
    ...
    while True:
        ...
        #     speed.linear.x = 0.2*sgm(-1*cur_y)*sgm(sin(cur_theta))
        #     speed.angular.z = 0
        #     v_ar.append(0.2*sgm(-1*cur_y)*sgm(sin(cur_theta)))
        #     w_ar.append(0)
        # else:
        break
        pub.publish(speed)
